<?php
	...
	class RevisionStore
	{
		...
		/**
		 * Fetches the count of revisions
		 * @return int|WP_Error
		 */
		public function fetch_revisions_count(): int|WP_Error
		{
			$res = $this->wpdb->get_var(
				$this->wpdb->prepare('SELECT COUNT(*) FROM ' . $this->table_name . ' WHERE post_id=%d', $this->post_id)
			);
			
			if ($this->wpdb->last_error) {
				return new WP_Error('fetch_count_error', $this->wpdb->last_error);
			}
